...
public class FolhaTextProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			FolhaTextProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				FolhaTextProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		if (target(3)) {
			if (Minecraft.getInstance().player != null) {
				...
				if (entity instanceof Player _plr0 && _plr0.containerMenu instanceof FolhaGUIMenu) {
					renderTexts((((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getOrCreateTag().getString("text")) + "\n" + "Semem"), 0, 0, 0, 0, 2, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
				}
			}
			release();
		}
